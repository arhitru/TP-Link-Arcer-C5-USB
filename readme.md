...
**Прошивка через Sysupgrade**
1. Стандартно загрузить файл "openwrt-...-sysupgrade.bin" через web-инткрфейс.

**Прошивка через Tftpd64**
1. Переименовать файл "openwrt-...-tftp-recovery.bin" в "tp_recovery.bin".
...
5. Закрываем настройку сети и снова открываем, убеждаемся, что адрес у нас 192.168.0.66.
6. Запускаем Tftpd64.
...
**Настройка USB и пакетов скриптами через Putty или MobaXterm**
1. Зайти в консоль через Putty или MobaXterm.
2. Подключить в Wan-порт кабель с раздачей интернета по DHCP. Можно через web-интерфейс или консоль настроить подключение роутера как клиента по Wi-Fi.
...
**Настройка USB и пакетов скриптами через PowerShell**
1. Открыть PowerShell
2. Подключить в Wan-порт кабель с раздачей интернета по DHCP. Можно через web-интерфейс настроить подключение роутера как клиента по Wi-Fi.
3. Установить USB-накопитель.
4. Вставить команду
```
ssh -o StrictHostKeyChecking=accept-new root@192.168.1.1 "cd /root && wget https://raw.githubusercontent.com/arhitru/TP-Link-Arcer-C5-USB/main/setup.sh && chmod +x setup.sh && ./setup.sh"
```
4. Подождать 15 минут. 
5. Скрипт сконфигурирует и подключит USB-накопитель, установит все пакеты и сделает необходимые настройки устройства.
